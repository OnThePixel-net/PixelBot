...
const starboard = require("../../Schemas/startboardSchema");
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((command) =>
      command
        ...
        .addChannelOption((option) =>
          option
            .setName("channel")
            .setDescription("The channel to send the starred messages into")
            .setRequired(true)
        )
    ...
    .addSubcommand((command) =>
      command
        .setName("check")
        .setDescription("Check the current starboard system status")
    ),
      ...
      case "setup":
        if (data) {
          return await sendMessage(
            `⚠️ Looks like the starboard system is already setup.. once a message gets \`${data.Count}\` ⭐'s, it will be sent in <#${data.Channel}>`
          );
        } else {
          ...
          await sendMessage(
            `🌍 I have setup the starboard system. Once a message gets \`${count}\` ⭐'s, it will be sent in ${channel}.`
          );
        }
        break;
      case "disable":
        if (!data) {
          return await sendMessage(
            `⚠️ Looks like there is no starboard system setup.`
          );
        } else {
